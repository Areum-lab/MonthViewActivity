...
public class MonthViewActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String[] date = new String[6*7]; //6*7사이즈의 String형 배열 선언

        for(int i=0, n=1; i<date.length; i++){
            if(i < START_DAY_OF_WEEK-1) //date배열에 첫번째 요일까지 공백으로 채움
                date[i] = "";           //DAY_OF_WEEK는 일요일:1 ~ 토요일:7로 결과가 나오기때문에 START_DAT_OF_WEEK에서 -1을 해준 값 전까지가 공백
            else if((i >= START_DAY_OF_WEEK-1) && (n <= END_DAY)) { //START_DAY_OF_WEEK에서 -1해준 값부터 배열은 시작
